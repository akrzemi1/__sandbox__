...
Static analyzers, in order to avoid false positive warnings, need to be positive that a given situation in a standard-conformant program is a bug. Hitting UB is a certain indication that there is a bug. But for this to work you have got to have the potential to hit UB. UB is close in nature to a [checksum](https://en.wikipedia.org/wiki/Checksum): it is redundant, increases size of a message, and creates the potential for producing invalid messages (where the payload doesn't match the checksum), so someone might sat "remove it, and there be no way to send an invalid message". But we still want to use checksums, and we all know why. 
