...
concept proto_allocator = std::same_as<Allocator, void> || (std::is_nothrow_copy_constructible_v<Allocator>  && requires(Allocator) {
    ...
    /*std::equality_comparable<Allocator> 
    && std::copy_constructible<Allocator> 
    && 
    requires(Allocator & a, T* p, size_t n)
{
    requires std::same_as<typename Allocator::value_type, T>;
    typename std::allocator_traits<Allocator>;
    typename std::allocator_traits<Allocator>::pointer;

    {a.allocate(n)} -> std::same_as<typename std::allocator_traits<Allocator>::pointer>;
    {a.deallocate(p, n) };
};*/
...
struct promise_base
{
    ...
    add_pointer_t<add_const_t<Ref>> value_;
    ...
    struct yield_value_holder {
        ...
        void await_suspend(coroutine_handle<Promise> h) noexcept {
            h.promise().value_ = addressof(ref);
        }
    ...
    template<typename T = std::remove_cvref_t<Ref>>
        requires is_convertible_v<T, Ref> && is_constructible_v<Ref, T> && (!same_as<remove_cvref_t<T>, Ref>)
    yield_value_holder yield_value(T&& x) {
    ...
    struct yield_sequence_awaiter {
        ...
        await_suspend(std::coroutine_handle<promise_type> h) noexcept {
            ...
            root.rootOrLeaf_ = coroutine_handle<promise_base>::from_address(gen_.coro_.address());
            nested.parent_   = coroutine_handle<promise_base>::from_address(h.address());
...
class generator {
  ...
  public:    
    ...
    class promise_type : public promise_base<Ref>, public promise_base_alloc<Alloc> {
      public:
        ...
        std::suspend_always yield_value(Ref x)  requires std::is_lvalue_reference_v<Ref> {
    ...
    typename promise_base<Ref>::template yield_sequence_awaiter<generator<Ref, OValue, OAlloc>> yield_value(elements_of<generator<Ref, OValue, OAlloc>> g) noexcept {
    ...
    typename promise_base<Ref>::template yield_sequence_awaiter<generator> yield_value(elements_of<R> r) {
