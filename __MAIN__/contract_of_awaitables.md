...
We choose to provice a ‘product’ return type (such as `tuple`) that in addition to carrying the number of read bytes, also provides a multi-purpose status code. This status code can be used to:
 ...
 2. Indicate that the framework decided to cancel the operation. This can happen as a consequence of:

     * calling `.cancel()` in another part of the program,
     * or a timeout.
  
    In this case you will get no bytes.
