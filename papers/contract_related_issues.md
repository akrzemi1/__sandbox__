...
This point of view is based on trust that compiler vendors work in the best interests of the programmers, that they are aware of the
issues related to assuming the CCS predicates, and that their natural course of action would be to either never implement such 
assumptions, or enable them under a compiler flag: not enabled by default, possibly with additional options to selectiely choose which
CCSs to use for assumptions. Under this view there is no problem related to contract-based optimizations in the current [[WD]][1] (except maybe for replacing unspecified behavior with implementation-defined behavior): it gives the compiler vendors a provision for implementing contract-based optimizations in a responsible way.
...
Expression `not std::greater<>{}(b, e)` is a runtime-checkable subset of a generally non-checkable `is_reachable(b, e)`. The latter implies the former.
...
This also illustrates that optimizations based on axiom-level CCSs can introduce new paths in the program (potentially with UB) in the same manner as default and audit level. If optimizations based on default- and audit-level CCSs cause concern, then so should the axiom-level ones. 

The current [[WD]][1] technically accommodates both positions but this is a superficial unification. Programmers and tool writers will need to be given instructions how to use axiom-level CCSs. And if they are given contradictory guidance this will create a fracture in the community even though the International Standard clearly defines what happens under each mode with axiom-level CCSs. This is because the use of CCSs is beyond what the IS can describe (like static analysis tools).

The consensus regarding the two positions needs to be reached even if it is not expressed directly in the IS. It can be used as a basis for applying fixes and future modifications to contracts. For instance, the solution in [[P1290r1]][3] takes the direction of the second position. It adds a new semantic distinction between axiom-level CCSs and other CCSs: the former are allowed to be used for UB-based optimizations under one mode, whereas other levels of CCSs are not allowed for UB-based optimizations under any defined mode. This distinction has no justification (based on the conceptual model) if we assume the first position (where axiom-level CCSs differ only in the guarantee that they will never be run-time evaluated). But it makes perfect sense if we assume the second position.
...
3\. How do we define "assume" for axiom-level CCSs?
---------------------------------------------------
...
This means that an implementaiton is allowed at its discretion to evaluate a condition inside an axiom-level CCS. This means that if my condition has side effects, and the implementation chooses to evaluate my condition, even though I used the `axiom` level, I can still get UB even if condition returns true.
