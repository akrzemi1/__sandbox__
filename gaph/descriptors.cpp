...
public:
    ...
    vertex_descriptor_t vertex_id(const vertex_nav_t& u) { return &u - _edge_ranges.data(); } 
    vertex_nav_t& vertex(vertex_descriptor_t const& uid ) { return _edge_ranges[uid]; }
...
public:
  ...
  VertexNav vertex(VertexDescriptor const& uid) const {
    return *uid;
  }
  
  VertexDescriptor descr(std::string const& s) {
...
public:
  ...
  VertexNav vertex(VertexDescriptor uid) const {
    return *uid;
  }
...
int main()
{
    auto print_uid = [](auto const& uid){ 
        if constexpr (std::is_pointer_v<std::decay_t<decltype(uid)>>)
            std::cout << uid->first << " ";
        else
            std::cout << uid << " "; 
    };
    ...
    }
    {
        ...
        g._vertices["i2"];
        g._vertices["i3"];
        g._vertices["i0"] = {"pld", {{"pld", g.descr("i1")}, {"pld", g.descr("i2")}}};
        g._vertices["i1"] = {"pld", {{"pld", g.descr("i0")}, {"pld", g.descr("i3")}}};
